...
def generate_mask(input_image, net, palette, device = 'cpu',):
    ...
    for cls in range(0, 4):  # Exclude background class (0)
    ...
    for cls in classes_to_save:
        ...
        imgClass.append(processBase64(alpha_mask_img))
        

    # Save final cloth segmentations
    cloth_mask = output_arr[0].astype(np.uint8) * 255
    cloth_seg = Image.fromarray(cloth_mask, mode='P')
    #cloth_seg.putpalette(palette)
    cloth_seg = cloth_seg.resize(img_size, Image.BICUBIC)
    imgClass.append(processBase64(cloth_seg))
    return imgClass
...
def process_masking(img64):
    ...
    #img = Image.open(img64).convert('RGB')

    img = Image.open(
        BytesIO(
            base64.b64decode(img64 )
        )
    ).convert('RGB')
